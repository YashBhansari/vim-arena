...
export class VimEngine {
    ...
    public onBeforeInsertChar?: (col: number, row: number) => boolean;
    public onBeforeReplaceChar?: (col: number, row: number) => void;
    ...
    private handleNormalMode(event: KeyboardEvent) {
        ...
        // Handle numeric counts for motions (e.g. 5j)
        if (/^[0-9]$/.test(key)) {
        ...
        switch (key) {
            ...
            case '$':
            case '0':
                // Standard motions with count support
    ...
    // Determines character category: Space (0), Word (1), or Special (2).
    ...
    private replaceChar(col: number, row: number, char: string) {
        if (this.onBeforeReplaceChar) this.onBeforeReplaceChar(col, row);
